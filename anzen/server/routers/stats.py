"""
GET /api/stats — aggregated metrics for dashboard overview.
"""
...
from anzen.server.database import get_db, EventModel
...
async def get_stats(db: AsyncSession = Depends(get_db)):
    ...
    # By guard type
    by_guard = await db.execute(
        select(EventModel.guard_type, func.count())
        .group_by(EventModel.guard_type)
    )

    # By category
    by_category = await db.execute(
        select(EventModel.category, func.count())
        .group_by(EventModel.category)
    ...
    timeline = await db.execute(
        select(EventModel.risk_score, EventModel.timestamp)
    ...
    return {
        ...
        "risk_timeline": [
            {"score": r.risk_score, "ts": r.timestamp.isoformat()}
            for r in reversed(timeline.all())
        ],
    }
