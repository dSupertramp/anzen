...
def wrap_mistral(client, config: AnzenConfig | None = None, session_id: str | None = None):
    """Wrap a Mistral client with Anzen. Returns an Anzen-wrapped client."""
    return _wrap(BaseAdapter(MistralCompletions(client)), config=config, session_id=session_id)
